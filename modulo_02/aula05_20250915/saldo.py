...
def depositar_valor(usuario:dict) -> None:
    """Permite ao usuario depositar um valor na sua conta."""
    try:
        valor_depositar=float(input("insira o valor aser depoistaot:"))
        if valor_depositar>0:
            usuario["saldo"]+=valor_depositar
            print("Depósito ralizado com sucesso!")
            verificar_saldo(usuario)
        else:
            print("Valor invalido")
    except ValueError:
        print("Erro! Poor favor entre apenas com números.")